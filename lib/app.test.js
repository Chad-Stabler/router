...
it('receives 404 when requesting unknown method cats', async () => {
  await request(app)
  .patch('/api/v1/cats')
  .expect(404);
});

it('receives 404 when requesting unknown method candies', async () => {
  await request(app)
  .patch('/api/v1/candies')
  .expect(404);
});

it('receives 404 when requesting unknown method sauces', async () => {
  await request(app)
  .patch('/api/v1/sauces')
  .expect(404);
});
